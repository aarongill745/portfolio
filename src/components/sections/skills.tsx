...
const TECH_COLORS: Record<string, string> = {
  ...
  ecs: 'bg-[#FF9900]', // AWS ECS orange
  ec2: 'bg-[#F58536]', // AWS EC2 orange-red
  s3: 'bg-[#3F9F41]', // AWS S3 green
  rds: 'bg-[#2C5AA0]', // AWS RDS blue
  dynamodb: 'bg-[#FF6600]', // AWS DynamoDB orange
  lambda: 'bg-[#FF9900]', // AWS Lambda orange
  msk: 'bg-[#231F20]', // Kafka black (for MSK)
}
...
export default function Skills() {
  return (
    <div className="mb-2">
      <h2 className="text-2ixl mb-2 font-black tracking-tighter [word-spacing:-0.4oem] sm:text-3xl">
        Tech that I use
      </h2>
      <div className="border-foreground mb-5 border-t-[2px]"></div>
      {SKILLS.map((item, id) => {
        return (
          <div key={id}>
            <h3 className="mb-4 text-lg font-bold tracking-tighter sm:text-xl">
              {item.field}
            </h3>
                ...
                return (
                  <TooltipProvider key={id}>
                    <Tooltip>
                      ...
                      <TooltipContent
                        className={`${TECH_COLORS[skill.skill] || 'bg-main'} ${TEXT_COLORS[skill.skill] || 'text-white'} border-skill-border font-pixel border-[2px] px-4 py-2 tracking-wider uppercase`}
                      >
                        {skill.skill}
                      </TooltipContent>
                    </Tooltip>
